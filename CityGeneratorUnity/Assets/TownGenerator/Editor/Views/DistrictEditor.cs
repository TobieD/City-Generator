...
namespace Tools.TownGenerator
{
    public class DistrictEditor
    {
        ...
        private void DistrictBuildingsGUI()
        {
            ...
            _selectedBuilding =
                (GameObject)
                    EditorGUILayout.ObjectField(_selectedBuilding, typeof(GameObject), false,
                        GUILayout.Width(buttonWidth * 1.5f));
            ...
            for (int i = 0; i < _buildingPrefabs.Count; ++i)
            {
                ...
                if (GUILayout.Button(prefabPreview, GUIStyle.none, GUILayout.Width(size + (size * 0.2f)),
