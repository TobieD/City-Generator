...
namespace Tools.TownGenerator
{
    ...
    public class TownBuilder
    {
        ...
        private void SpawnTree(Vector3 position)
        {
            ...
            //random tree from the prototypes
            var treeIndex = Random.Range(0, _terrain.terrainData.treePrototypes.Length);

            //Get the height of the terrain at the spawn position
            position.y = _terrain.SampleHeight(position);

            //check if a tree overlaps with anything
            if (isOverlapping(_terrain.terrainData.treePrototypes[treeIndex].prefab, position))
            {
                return;
            }
