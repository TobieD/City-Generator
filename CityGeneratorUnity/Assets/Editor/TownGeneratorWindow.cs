...
public class TownGeneratorWindow : EditorWindow
{
    ...
    [MenuItem("TownGenerator/Creator")]
    ...
    private void CitySettings()
    {
        ...
        //General Settings
        _citySettings.GenerateInnerRoads = EditorGUILayout.Toggle("Generate inner roads",_citySettings.GenerateInnerRoads);

        if (_citySettings.GenerateInnerRoads)
        {
            _citySettings.RoadSubdivision = EditorGUILayout.IntSlider("Road subdivision", _citySettings.RoadSubdivision,
                1, 2);
        }
    ...
    private GenerationSettings GetSettingsFromPreset(CityType cityType)
    {
        ...
        switch (cityType)
        {
            ...
            case CityType.Town:
            {
                genSettings.Width = 500;
                genSettings.Length = 500;
                genSettings.PointAlgorithm = PointGenerationAlgorithm.Simple;
    ...
    private void SaveAsXml(string filename)
    {
        ...
        var grassElem = new XElement("GrassDetail");
        foreach (var grass in _terrainEditor.GetSettings().GrassTextures)
        {
            var path = AssetDatabase.GetAssetPath(grass.Texture);
            grassElem.Add(new XElement("Grass", path));
    ...
    private void LoadFromXml(string filename)
    {
        ...
        var grass = new List<GrassTexture>();
        foreach (var e in terrain.Elements("GrassDetail").Elements())
        {
            var grassTex = new GrassTexture()
            {
                Texture = AssetDatabase.LoadAssetAtPath<Texture2D>(e.Value),
            };
            grass.Add(grassTex);
        ...
        _terrainEditor.GetSettings().GrassTextures = grass;
